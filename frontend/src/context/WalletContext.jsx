...
const hasWeb3AuthClientId = Boolean(web3AuthClientId);
...
const walletDisabledValue = {
  ...
  connectorName: null,
  login: async () => {
    throw new Error('Web3Auth is disabled because VITE_WEB3AUTH_CLIENT_ID is not configured.');
  },
  logout: async () => {},
  signMessage: async () => {
    throw new Error('Web3Auth is disabled because VITE_WEB3AUTH_CLIENT_ID is not configured.');
  ...
  if (!hasWeb3AuthClientId) {
    return (
      <QueryClientProvider client={queryClient}>
        <WalletContext.Provider value={walletDisabledValue}>{children}</WalletContext.Provider>
      </QueryClientProvider>
    );
  }
